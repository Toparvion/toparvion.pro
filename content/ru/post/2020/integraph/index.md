...
{{< figure src="export/style-panel.png" title="Выделенная оранжевым кнопка сбрасывает стиль в умолчательный" lightbox="true" >}}
...
{{< icon name="download" pack="fas" >}} *[Скачать запрос файлом](export/integraph.cypher)*
...
1. Создаём/обновляем каждый узел, безусловно присваивая ему метку `Node` (чтобы между узлами всё же осталось что-то общее), а также вторую метку со значением:
   - `unknown`, если у данного узла вообще не указан тип (что, конечно, дичь, но иногда бывает);
   - `channel`, если полное название типа заканчивается на слово `channel`, например: `null-channel`;
