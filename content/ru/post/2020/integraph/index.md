...
subtitle: "Строим мост из мира Java Enterprise в мир графовых СУБД"
summary: "Enterprise Integration Patterns давно имеют много общего с теорией графов, а графовые СУБД вроде Neo4j отлично справляются с визуализацией и анализом таких структур данных. Кажется, пора их познакомить..."
authors: 
  - toparvion
tags:
  - spring-integration
...
lastmod: 2020-04-26T10:05:05+07:00
featured: false
draft: false
...
1. Выполнить [этот](#расширенный-запрос) Cypher-запрос в Neo4j, подставив туда URL.
1. Профит 😋
...
Статья рассчитана на тех, кто знаком с основами Neo4j и языка Cypher. Но если нет, не беда – в тексте есть ссылки на поясняющие источники.
...
{{< icon name="exclamation-triangle" pack="fas" >}} *Каналы в терминах EIP **не** являются связями в графе!*  
...
1. С помощью [APOC-процедуры](https://neo4j.com/docs/labs/apoc/current/import/load-json/) `apoc.load.json(url)` мы загружаем JSON-граф целиком и для краткости и наглядности переименовываем его корень в `json`, а объект с дескриптором – в `jsonDescriptor`.
...
В общем-то, цель достигнута: EIP-компоненты превращены в узлы графа, между узлами проведены связи в нужных направлениях, а если поводить курсором по элементам графа в Neo4j Browser, то в нижней части панели будут отображаться всякие дополнительные свойства, которые мы перекладывали в Cypher-запросе:
...
И вот теперь, с учётом перечисленных пожеланий, наш прежний запрос может выглядеть примерно так:[^2]
