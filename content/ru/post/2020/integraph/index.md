...
И вот теперь, если переписать предыдущий Cypher-запрос с учётом всех перечисленных пожеланий, получится примерно так:
...
1. Создаём/обновляем связи, на сей раз выбирая тип на основе поля `type` из JSON-графа и приводя его к верхнему регистру для соответствия лучшим практикам графовых БД.
...
Пока в нашей БД всего один граф, вывести его можно даже простейшим запросом типа `MATCH (a) RETURN a`, но из-за искусственной связи дескриптора с каждым узлом граф будет выглядеть запутанным. Поэтому лучше вывести его не целиком, а так, чтобы в результат попали все узлы, относящие к данному дескриптору, но не он сам. А сам дескриптор проще всего найти по имени приложения (поле `name`):
...
Чтобы этого избежать, нужно либо снабдить каждый узел дополнительным отличительным свойством (хотя это подход в духе реляционных СУБД), либо в самом запросе учесть наличие связи с нужным дескриптором.
...
В поставке Neo4j есть отличная утилита `bin/neo4j-admin`, которая при помощи команды `memrec` умеет подсказывать, какой и сколько памяти нужно прописать в конфигурации, чтобы жить хорошо.

Будучи правильно поставленной на службу, Neo4j может стать добрым помощником в визуализации графов приложений на Spring Integration. Это особенно ценно, когда эти графы искажены или запутаны, как например те, с которыми мне доводилось встречаться:
