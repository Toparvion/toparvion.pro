...
В целом, цель достигнута: EIP-компоненты превращены в узлы графа, между ними проставлены связи с нужными направлениями, а если поводить курсором по элементам графа в Neo4j Browser, то в нижней части панели будут отображаться всякие дополнительные свойства, которые мы перекладывали в Cypher-запросе:
...
Если такие недостатки не смущают, то дальше можно не читать, а если дух перфекциониста всё же взбунтовался, то нам понадобится…

#### Подход 2: визуализация с предобраткой
