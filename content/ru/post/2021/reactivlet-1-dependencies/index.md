...
Разумеется, сам по себе последний вариант не позволит традиционному “сервлетному”коду работать на реактивном стеке. Но зато он наставит на истинный путь упомянутый выше метод `WebApplicationType#deduceFromClasspath`. Останется только починить всё то, что поотваливается ~~на фиг~~ от такого переключения. Этому увлекательному занятию и будут посвящены последующие заметки. И ближайшая из них [раскроет]({{< relref "/post/2021/reactivlet-2-mdc" >}}) тему логирования.
