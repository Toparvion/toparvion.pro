---
title: "От сервлетов к реактору"
subtitle: "Часть 1: введение и зависимости"
summary: "Часть 1: введение и зависимости"
authors:
  - toparvion
tags:
  - spring
...
Разумеется, само по себе такое решение не позволит традиционному “сервлетному”коду работать на реактивном стеке. Но зато оно наставит на истинный путь упомянутый выше метод `WebApplicationType#deduceFromClasspath`. Останется только починить всё то, что поотваливается ~~на фиг~~ от такого переключения. Этому увлекательному занятию и будут посвящены последующие разделы. И первый из них раскроет тему логирования.
...
Чтобы можно было заполучить текущий запрос практически из любой точки ~~мира~~ прикладной логики, не пробрасывая его от сáмого контроллера, можно применить тот же подход, который был описан в [разделе про MDC](#дополнение). Суть подхода примерно в следующем:
...
Поскольку подробный вывод подобного класса уже приводился в ==разделе про MDC==, здесь будет сразу представлен полный код класса:
...
Так, например, в прилагаемом ==демо-проекте== в модуле `shared` есть метод `UnifiedController#inspect()`, как раз содержащий такой фрагмент кода и возвращающий его результаты в виде:
